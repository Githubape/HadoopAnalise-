 ...
 * @Version 1.0.0
 */
public class ParentsSatisDriver {
     /*
    Map处理
    分割及映射数据
     */

    public static class ParentsSatisMapper extends Mapper<LongWritable, Text,Text, IntWritable>
    {
        @Override
    ...
    汇总数据
     */
    public static class ParentsSatisReduce extends Reducer<Text,IntWritable,Text,IntWritable>
    {
        @Override
    ...
    public void run(String in,String out) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        //指定jar包
        job.setJarByClass(ParentsSatisDriver.class);
        //指定Mapper Reducer业务处理类
        job.setMapperClass(ParentsSatisMapper.class);
        job.setReducerClass(ParentsSatisReduce.class);
        //指定map输出类型
...
//                "hdfs://hadoop46:9000/data/userimgs/output/parentsSatisanly"));
